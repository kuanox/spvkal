...
function LoadResetValor1(limit)
{
	//console.log(limit);
	...
	setTimeout(function(){LoadResetValor1()},2000);
	ajaxMenu=objetoAjax(); 
...
function LoadSetValoresGrafico(fechaHoraBuscar){
			...
			if(respuesta){
				...
				for (z=0; z<alldata.length; z++){
					...
					for (i=0; i<(alldata[z][1]).length; i++){
						...
						horamin = hora + min + seg;
						alldata[z][1][i] = new Array(parseFloat(horamin),parseFloat(temp[1]));
					}
	...
	ajaxMenu2.send("trig="+index+"&limit="+"'"+fechaHoraBuscar+"'");
...
function LoadSetUltimoValorGrafico(fechaHoraBuscar){
		...
		if (ajaxMenu3.readyState==4) {
			...
			if(respuesta){
				...
				for (i=0; i<temp.length; i++){
						...
						horamin = hora + min + seg;
						datoIngresar[i] = new Array(temp[i][0], parseFloat(horamin), parseFloat(temp[i][1]));
	...
	ajaxMenu3.send("trig="+index+"&limit='"+fechaHoraBuscar+"'");
